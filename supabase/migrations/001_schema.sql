-- OneRonnoco Migration 001: Full Schema
-- Run in order on new Supabase project

-- ============================================================
-- SECTION 1: ROMs and Users
-- ============================================================
...
CREATE TABLE public.users (
  ...
  email TEXT NOT NULL,
  full_name TEXT NOT NULL,
  role TEXT NOT NULL CHECK (role IN ('admin','director','sales_rep','ops','external')),
  rte_number TEXT,
...
CREATE TABLE public.distributor_divisions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE public.deal_equipment (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE public.distributor_mapping_profiles (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  distributor_id UUID NOT NULL REFERENCES public.distributors(id),
...
CREATE TABLE public.velocity_uploads (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  distributor_id UUID NOT NULL REFERENCES public.distributors(id),
  division_id UUID REFERENCES public.distributor_divisions(id),
...
CREATE TABLE public.velocity_records (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_velocity_period ON public.velocity_records(period_year, period_month);
CREATE INDEX idx_velocity_customer ON public.velocity_records(customer_id);
CREATE INDEX idx_velocity_distributor ON public.velocity_records(distributor_id);
CREATE INDEX idx_velocity_product ON public.velocity_records(product_category);
